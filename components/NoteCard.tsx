...
export default function NoteCard({ note, onDelete }: NoteCardProps) {
    ...
    return (
        ...
        /**
         * some of my STYLING EXPLANATION:
        ...
        <article
            className={`group relative rounded-xl shadow-md px-3 py-2 min-h-[90px] min-w-[140px] ${colorClass}  font-['Edu_VIC_WA_NT_Beginner'] border border-black/10`}>

            {/* underline effect for the saved notes*/}
            {/* inset-0:fill the whole card from edge to edge*/}
            {/* pointer-events-none:makes this layer ignore mouse interactions.*/}
            {/* bg-[linear-gradient(to_bottom,transparent_95%,rgba(0,0,0,0.08)_100%)]: creates a repeating gradient that adds a line near the bottom of each row.*/}
            {/* bg-[length:100%_1.6rem]: controls how frequently the gradient repeats vertically.*/}
